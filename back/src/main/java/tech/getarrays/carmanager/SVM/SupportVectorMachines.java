...
public class SupportVectorMachines {
	...
	private int performSMO() {
		...
		for (int i = 0; i < x.getData().length; i++) {
			...
			if (checkAlphaVioletesKKT(alpha.getEntry(i,0), Ei.getEntry(0, 0))) {
	...
	private boolean optimize1AlphaPair(int i, int j, double Ei, double Ej,
		...
		if(Math.abs(alpha.getEntry(j, 0) - alphaJold) >= MIN_ALPHA_OPTIMIZATION ){
			opimizeAlphaISameAsAlphaJOppositeDirection(i,j,alphaJold);
			flag = true;
		}
		return flag;
		
	}
	private void opimizeAlphaISameAsAlphaJOppositeDirection(int i, int j, double alphaJold) {
	...
	private boolean checkAlphaVioletesKKT(double alpha, double e) {
	...
	private int selectIndexOf2ndAlphaToOptimize(int indexOf1stAlpha, int numbOfRows) {
		int indexOf2stAlpha = 0;//
		 indexOf2stAlpha = indexOf1stAlpha;
		while(indexOf1stAlpha == indexOf2stAlpha) indexOf2stAlpha = ThreadLocalRandom.current().nextInt(0, numbOfRows-1);
		
		return indexOf2stAlpha;
	...
	public String classify(RealMatrix entry) {
		String classification ="classified as -1 (very good car)";
		if(Math.signum(entry.multiply(w).getEntry(0,0) + b) == 1) {
			classification ="classification as 1 (not good car)";}
		   
		return classification;
	}
	public RealMatrix mult(RealMatrix matrix1,RealMatrix matrix2) {
